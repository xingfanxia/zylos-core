...
Three mechanisms prevent instances from confusing user identities:
...
2. **Instance-scoped conversation queries** — `c4-session-init.js` uses `getUnsummarizedConversationsForInstance()` from `c4-db-multi.js` which filters by `target_instance`. If the import fails, conversation injection is skipped entirely (fail-safe, not fail-open).
