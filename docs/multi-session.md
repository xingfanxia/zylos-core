...
2. **Instance-scoped conversation queries** — inbound routing and outbound sends both record the owning instance in `target_instance`. `c4-session-init.js` uses `getUnsummarizedConversationsForInstance()` from `c4-db-multi.js`, which filters strictly on `target_instance = ?` (no `OR target_instance IS NULL` fallback). Each instance therefore restores both sides of its own conversations without reading peer or legacy unscoped rows. If the import fails, conversation injection is skipped entirely (fail-safe, not fail-open).
