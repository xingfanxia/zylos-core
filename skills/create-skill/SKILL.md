...
New skills MUST be created at the **project-level** skills directory:

```
~/zylos/.claude/skills/<skill-name>/
```
...
When creating a new skill from scratch, always run the `init-skill.js` script. It generates a new template skill directory with everything a skill requires.
...
Default output path: `~/zylos/.claude/skills/`. Override with `--path`:

```bash
node scripts/init-skill.js <skill-name> --path ~/.claude/skills
```
...
To validate a skill: `node scripts/validate.js <skill-directory>`
